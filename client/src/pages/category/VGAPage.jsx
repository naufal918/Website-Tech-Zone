...
import { Gpu } from 'lucide-react'; // ikon Gpu biar sesuai
...
export default function RouterPage() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const res = await axios.get("http://localhost:5000/products/category/vga");
        const sorted = res.data.sort((a, b) => b.price - a.price);
        console.log("Data produk router:", res.data);
        setProducts(sorted);
      } catch (error) {
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex items-center gap-2 mb-8 justify-center">
        <Gpu className="w-7 h-7 text-blue-600" />
        <h2 className="text-3xl font-bold text-black-600">VGA/GPU</h2>
      </div>
